...
c
c  commons for parameter transmission   ! set implicit none, declare all variables explicitly
!  declare both ipara_* and *, such that the parameters passed out from res_adipar (after being converted to *) are also declared in the same type 
c
      common/cadr_param/
     *  para_el, para_els1, para_dels, para_dfsig1, para_dfsig2,  ! el, els1, dels changed from para to ipara -> integer 
     *  para_sig1, para_sig2, para_dfsig, para_eltrw1, para_eltrw2,
     *  para_sgtrw1, para_sgtrw2 
!      integer ::  
!     *   ipara_el, ipara_els1, ipara_dels,    ! real(dp) in original ver, change to integers to match mesa input and output settings
!     *   el, els1, dels
!      real(dp) :: 
...
c
     ...
     *  ipara_istsig, ipara_inomd1, ipara_iscan ! irotkr in common/coutpt/
!      integer ::  
!     *  ipara_nsel, ipara_nsig1, ipara_nsig2, ipara_itrsig, ipara_nsig, 
...
!      real(dp) :: &
!        xmod1,datsum(8),datmd1(2),xtrnc1,dum13,xfit1, &
!        fsbcsm,fcbcsm,albsum, &
!        elsum,ordsum,sigsum,sigc,dmx,xmx,ekin,per,perv,frqv, &
!        ddsig,ddsol,ysum(4),dosdum(5)
!      integer :: 
...
C
     ...
     *  aa(iaa,1)
!      real(dp) :: ell,alb,els,el1,sig,anres,perfac,data(8), aa(iaa,1)
!      integer :: el
c
      common/rotdat/ ipara_iem, ipara_irotsl, ipara_nsem, ipara_idems  ! ems1 not needed for now
!      integer :: ipara_em, ipara_irotsl, ipara_nsem, ipara_dems, &
!        em, irotsl, nsem, dems

!  common containing angular velocity omega0, added by me 
      common/rotvel/ para_omega0
!      real(dp) :: para_omega0, omega0
c
      common/bcsdat/ fctsbc, fcttbc, istsbc, ibotbc
!      real(dp) :: fctsbc, fcttbc
!      integer :: istsbc, ibotbc
c
      common/nrmchk/ nnww,ncfac,irsdif
!      integer :: nnww,ncfac,irsdif
c
...
!      real(dp) :: &
!        xmnevn,xfit, fcnorm, eps, epssol, dsigmx, fsig, dsigre
!      integer :: &
!        icow,iturpr,iplneq,iriche,nnwwin, moddet, itmax, irsevn, 
!        nftmax,irsord,iekinr,itsord,imissl,imjssl,imstsl,inomd1
c
...
c
     ...
     *  nnq1,nnq2,idgnrk
!      real(dp) :: cbc(4,4),cavc(4,4),cavs(4,4)
!      integer :: isngbt,isngsf,ibcnr1,nnq1,nnq2,idgnrk
c
      common/sysord/ ii
!      integer :: ii
c
...
!      real(dp) :: y(iy,1)
c
      common/varcon/ ipara_ivarf, ipara_npvarf, ipara_kvarfc
!      integer :: ipara_ivarf, ipara_npvarf, ipara_kvarfc, &
!        ivarf, npvarf, kvarfc 
c
      common/cmdtst/ iordtr,icaswn, sigwn1, sigwn2, frqwn1, frqwn2, 
     *  iorwn1, iorwn2, frlwn1, frlwn2
!      integer :: iordtr,icaswn, iorwn1, iorwn2
!      real(dp) :: sigwn1, sigwn2, frqwn1, frqwn2, frlwn1, frlwn2
c
      common/cprcns/ epsprc,epsufl,epsofl,eprufl
!      real(dp) :: epsprc,epsufl,epsofl,eprufl
c
      common/cofile/ nfiles, idsfil(99), file(99), iopen(99), filess(99)
!      integer :: nfiles, idsfil(99), iopen(99)
!      real(dp) :: file(99), filess(99)
c
      common/cwindw/ eltrw1, eltrw2, sgtrw1, sgtrw2
!      real(dp) :: eltrw1, eltrw2, sgtrw1, sgtrw2
c
c  common for storage of modal parameters (degree, order, cyclic frequency,
c  inertia, beta, split1, split2,split3)
c
...
!      integer :: idgrhs, idgrh1, iprdet, itssol, idgtss
c
...
c
     ...
     *  idsrkr,idsgkr,itrds
!      integer :: imlds,idslog,idsgsm,idsssm,idsefn, &
!        idsrkr,idsgkr,itrds
c
c
...
c
      ...
      write(*,*) '**********************************************'
      write(*,*) ''
      write(*,*) 'Reading adipls.c.d'  
      write(*,*) ''
      write(*,*) '**********************************************'
      write(*,*) ''
      write(*,*) ''
      write(*,*) 'Show me irotsl, em, irotkr, omega0 in adipls.c.d'
      write(*,*) 'in common blocks passed from mesa' 
      write(*,*) ipara_irotsl, ipara_iem, ipara_irotkr, para_omega0
      write(*,*) '' 
      write(*,*) '' 
! later called res_adipar to convert ipara_irotsl into irotsl etc.  
...
c
      ierr_param=-1
c
      write(*,*) 'iem at adipls.c.d line 598 =', iem
      write(*,*) 'ispcpr at adipls.c.d line 598 =', ispcpr
c
...
c  nsem .gt. 0: take nsem steps, starting from ems1, with step dems
      ...
      ems1=0
      idems=1 !0
!  controls for rotation rate, added by me 
      omega0=3.0
c
...
c
      ...
      write(istdou,*) 
     *  ' Trial freq input on unit itrds (default 10) (optional).' 
...
c
      if(ircrot.gt.0) then
c
        write(istdou,*) ' iem, irotsl, nsem, ems1, idems, omega0?'    ! em and irotsl switched
        write(istdou,*)   iem, irotsl, nsem, ems1, idems, omega0
        read(istdin,*,end=85,err=90) iem, irotsl, nsem, ems1, idems, omega0  ! where is this reading from, common or adipls.c.in? 

        write(*,*) 'iem at adipls.c.d line 1258 =', iem    ! for testing
c
      end if
c
      if(irccst.gt.0) then
...
c
     ...
     *                iorwn1, iorwn2, frlwn1, frlwn2
c
        write(*,*) 'irotkr, ispcpr at adipls.c.d line 1220 =', irotkr, ispcpr    ! for testing
c
      end if
c
      if(ircdgn.gt.0) then
...
c
        ...
        write(*,*) ''
        write(*,*) 'Show me irotsl, iem, nsem, idems, irotkr,'
        write(*,*) 'omega0 after call res_adipar (line 1266)'
        write(*,*) 'with i_param=1' 
        write(*,*) irotsl, iem, nsem, idems, irotkr, omega0 
        write(*,*) ''
c
        return
c
...
c
      write(*,*) 'iem at adipls.c.d line 1367 =', iem    ! for testing
      write(*,*) 'irotkr at adipls.c.d line 1367 =', irotkr
c
c  ***************************************************************
c
...
10000 continue
c
      write(*,*) 'iem at adipls.c.d line 1377 =', iem    ! for testing
      write(*,*) 'irotkr at adipls.c.d line 1284 =', irotkr
c
...
c      
        write(*,*) ''
        write(*,*) 'Show me irotsl, iem, nsem, idems, irotkr,'
        write(*,*) 'omega0 after call res_adipar (line 1317)' 
        write(*,*) 'with i_param=2' 
        write(*,*) irotsl, iem, nsem, idems, irotkr, omega0   ! em became 1.0000009536743 here
        write(*,*) ''

      end if
c
      write(*,*) 'iem at adipls.c.d line 1425 =', iem    ! for testing
      write(*,*) 'irotkr at adipls.c.d line 1426 =', irotkr
...
c
     ...
     * omega0)  ! added by me 
      write(*,*) ''
      write(*,*) 'Show me irotsl, iem, nsem, idems, irotkr,'
      write(*,*) 'omega0 after call res_adipar (line 1389)' 
      write(*,*) 'with i_param=1' 
      write(*,*) irotsl, iem, nsem, idems, irotkr, omega0 
      write(*,*) ''
c
...
c
      if(ircrot.gt.0) then
        write(istdou,*) ' irotsl, iem, nsem, ems1, idems, omega0'
        write(istdou,*)   irotsl, iem, nsem, ems1, idems, omega0, tail
c
      end if
c
      if(irccst.gt.0) then
...
c
      write(*,*) 'iem at adipls.c.d line 1652 =', iem    ! for testing
c
...
c
	if(irotkr.eq.11.or.irotkr.eq.21) then
	  call set_rotation(x, nn0)
!	  if(init_obsrot.eq.0) read(istdin,'(a)') obsrot_file
	  init_obsrot=1
        end if
      end if
c
      write(*,*) 'iem at adipls.c.d line 1740 =', iem    ! for testing

   ! Call set_rotation regardless of i_inout and i_find   ! added by me 
      if(irotkr.eq.11.or.irotkr.eq.21) then

	write(*,*) 'Calling set_rotation from adipls.c.d' 

        write(*,*) 'Show me irotsl, iem, irotkr, nn0 before' 
        write(*,*) 'calling set_rotation in adipls.c.d'
        write(*,*) irotsl, iem, irotkr, nn0 
...
c  *******************
c
      write(*,*) 'iem at adipls.c.d line 1969 =', iem    ! for testing

      if(irotsl.eq.1.and.nsem.ne.0) then
        if(nsem.eq.-1) then
          write(*,*) 'show me iem before iem test step (line 1973)'
          write(*,*) iem 
	  iem=-el
        else
	  iem=ems1
...
c
      write(*,*) 'iem at adipls.c.d line 1988 =', iem    ! for testing

      if(irotsl.eq.1.and.iem.gt.el) then
        write(istdou,127) iem, el
        if(istdou.ne.istdpr.and.istdpr.gt.0) write(istdpr,127) iem, el
	go to 80
      end if
c
c  new sigma**2
...
c
      write(*,*) 'iem at adipls.c.d line 2037 =', iem    ! for testing

      if(irotsl.ne.1) then
        write(istdou,180) jstsig, el, sig
      else
	write(*,*) 'iem for start new mode=', iem   ! for testing 
        write(istdou,182) jstsig, el, iem, sig
      end if
	  ...
	  write(*,*) 'iem for start new mode=', iem   ! for testing 
	  write(*,131) el, iem   ! for testing 
          write(istdpr,131) el, iem
        end if
      end if
c
c  zero part of /csumma/ containing results from previous mode
c
      call zero(csummm(18),21) ! for degree
      ! don't do it for em!!
c
...
c  test for scan 
c
      write(*,*) 'iem at adipls.c.d line 2117 =', iem    ! for testing
c
c
      if(iscan.le.1) then
c
c  normal iteration
c
	write(*,*) 'calling sigsol'
	write(*,*) 'show me iem before calling sigsol'
	write(*,*) iem
c
...
c
	write(*,*) 'calling sigout' 
	write(*,*) 'show me iem before calling sigout' 
	write(*,*) iem 
c
        call sigout(sig,x,y,iy,nw1,nibc,nn,nnw,mdintg,nev1,nfit,
     *    istsb1,iord,isolcv,isigcv,
     *    iem, irotsl)  ! rots added by me, sig removed 
c
      else
c
	write(*,*) 'calling sigscn' 
	write(*,*) 'Show me irotsl, iem, nn before calling sigscn'
	write(*,*) irotsl, iem, nn

        call sigscn(sigs1,sigs2,iscan,nsig,itrsig,x,y,iy,
     *    nw1,nibc,nn,nnw,mdintg,nev1,nfit,istsb1,icry,
     *    iem, irotsl)  ! added by me, pass iem, irotsl, ispcpr as arguments
c
        if(icry.lt.0) then
	  if(i_paramset.le.0) then
	    go to 6
...
c
      write(*,*) 'iem at adipls.c.d line 2178 =', iem    ! for testing
c
      if(irotsl.eq.1.and.nsem.ne.0) then
	write(*,*) 'Show me iem, idems in continue step (line 2085)'
	write(*,*) iem, idems 
        if((nsem.eq.-1.or.nem.lt.nsem).and.iem.le.el-idems+1.d-10) then
	  nem=nem+1
	  iem=iem+idems
	  write(*,*) 'Show me iem after continue step (line 2186)'
	  write(*,*) iem
c
...
c
      write(*,*) 'iem at adipls.c.d line 2205 =', iem    ! for testing
c
...
c
      if(init_obsrot.eq.1) call dump_obs(strcompr(obsrot_file))  ! to process obs_st results from setobs_st from rotker
c
c
...
!     *  ' trial sigma**2 =', 1pe12.4)
  182 format(/i5,'. Start iteration for l =',f10.3,' m =',i10.1,
     *  ' trial sigma**2 =', 1pe12.4)
...
c
     ...
     *  aacom(iaa,1)
      common/rotdat/ iem, irotsl, nsem, idems 
      common/rotvel/ omega0   ! added by me 
      common/cprcns/ epsprc,epsufl,epsofl,eprufl
c
c  common defining file input and output
c
...
c..     *  xfit1,xfitst
c
      write(*,*) 'Subroutine setlog called'
...
c
      common/cstdio/ istdin, istdou, istdpr, istder
c
      write(*,*) 'Subroutine testri called'
...
c
      common/cstdio/ istdin, istdou, istdpr, istder
c
      write(*,*) 'Subroutine dmpfls called'
...
c
      ...
      write(*,*) 'Subroutine dump_csumma called'
