...
!  set implicit none, declare all parameters 
...
c
      save
c
!      write(*,*) 'Subroutine adirhs called'
...
c
      if(irotsl.eq.1.and.iem.ne.0) then
        sigom2=1.5d0*(aa(10,n)-1.d0)*aa(1,n)
        sigom=sqrt(sigom2)
	sighat=sqrt(sig)+iem*sigom
        sigfct=sighat*sighat/sig
	alpha=2.d0*iem*sigom/sighat
