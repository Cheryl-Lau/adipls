...
c
      write(*,*) 'Subroutine dump_obs called'
c
c
c  test for no data
c
...
c
c  write to output file (not mesa)
c
      obs_file = 'outputs_with_splittings.txt'  ! added by me 

      open(99,status='unknown',file=obs_file,form='formatted')

      write(99,110)  ! dump contents
      ...
      if(icobs_st0 .eq. 1) then
        write(99,120) 'Variational frequency'  ! frequency from:
...
c
	...
	write(99,125)   ! write header degree, order, em, freq, inertia
	imax=5 !4
      else if(icobs_st1.gt.1) then
         write(99,129)  ! write header degree, order, em, freq, inertia, beta, split1, split2, split3
         imax=9 !8
      else
	write(99,127)   ! write header degree, order, em, freq, inertia, beta, split1
	imax=7  !6
      end if
c
c
c   write out results here
c
      do n=1,nobs_st
c
        ll=intgpt(obs_st(1,n)+0.5)   ! degree
        nord=intgpt(obs_st(2,n)+0.5) ! order 
c
...
!  125 format('# degree, order, frequency (microHz), inertia'/'#')
  125 format('# degree, order, em, frequency (microHz), inertia'/'#')
!  127 format('# degree, order, frequency (microHz), inertia, ',
!     *   ' beta, splitting (microHz)'/'#')
  127 format('# degree, order, em, frequency (microHz), inertia, ',
     *   ' beta, splitting (microHz)'/'#')
!  129 format('# degree, order, frequency (microHz), inertia, ',
!     *   ' beta, splitting1 (microHz), splitting2 (microHz), ',
!     *   ' splitting3 (microHz) '/'#')
  129 format('# degree, order, em, frequency (microHz), inertia, ',
     *   ' beta, splitting1 (microHz), splitting2 (microHz), ',
     *   ' splitting3 (microHz) '/'#')

c write degree, order as integers, cs(38) [em] in real
!  130 format(2i5,f10.3,1pe13.5,0pf10.6,1pe13.5,1pe13.5,1pe13.5)
  130 format(2i8,f8.1,f20.5,1pe20.5,0pf20.6,1pe20.5,1pe20.5,1pe20.5)
c
c
      end
