      subroutine setobs_st(cs,ics,iriche, iem)
...
!      implicit none 
      ...
      integer, intent(in) :: iem  ! added by me 

      common/rhsdat/ el,ell,alb,els,el1,sig,anres,perfac
!      common/rotdat/ iem, irotsl, nsem, idems   ! added by me 
...
c  degree, order, iem, cyclic frequency (microHz), inertia
c
...
c
      if(nobs_st.eq.0) then
        if(ispcpr.eq.0.or.ispcpr.eq.5) then ! No special output or do Richardson extrapolation freq
	  if(iriche.eq.1) then  
	    icobs_st0=5           ! ispcpr- do Richardson extrapolation freq
	  else if(iper.eq.1) then
	    icobs_st0=1           ! variational freq of richardson extrapolation freq
	  else
	    icobs_st0=6
	  end if
        else if(ispcpr.eq.1) then  ! ispcpr- do variational freq
	  if(iper.eq.1) then    ! variational freq
	    icobs_st0=1
	  else
	    icobs_st0=6
	  end if
        else                       ! ispcpr- eigenfreq in cs(20) or eigenfreq in cs(21)
	  icobs_st0=ispcpr         ! icobs_st0 = ispcpr = 6 for possibly corrected eigenfreq
        end if
c
...
c
      nobs_st=nobs_st+1
      obs_st(1,nobs_st)=cs(18) ! degree
      obs_st(2,nobs_st)=cs(19) ! order
      obs_st(3,nobs_st)=cs(38) ! iem, added by me

      write(*,*) 'iem in setobs_st =', iem   ! checking 
      write(*,*) 'cs(38) in setobs_st =', cs(38)  ! checking 

      obs_st(5,nobs_st)=cs(24) ! inertia 
      if(icobs_st0.eq.5) then
        obs_st(4,nobs_st)=1000d0*cs(37)  ! cyclic freq from Richardson extrapolation 
      else if(icobs_st0.eq.1) then
        obs_st(4,nobs_st)=1000d0*cs(27)  ! cs(27): cyclic frequency (mHz)
      else if(icobs_st0.eq.6) then
        obs_st(4,nobs_st)=sqrt(cs(21))/(60.d-6*perfac) ! cs(21): (possibly corrected) dimensionless eigenfrequency sigma**2  !!! use this?
                                                       ! since istsig=1, sig1 sig2 already in dimensionless eigenfreq sigma**2
      else 
        obs_st(4,nobs_st)=sqrt(cs(20))/(60.d-6*perfac) ! cs(20): dimensionless eigenfrequency sigma**2 
          
             ! obs_st(4,nobs_st) converted to 
...
c
      if(irotkr.eq.11) then
	obs_st(6,nobs_st)=beta
	obs_st(7,nobs_st)=split(1)
      end if
      if(irotkr.eq.21) then
	obs_st(6,nobs_st)=beta
	obs_st(7,nobs_st)=split(1)
        obs_st(8,nobs_st)=split(2)
        obs_st(9,nobs_st)=split(3)  
      end if      

!  all numbers after 3 shifted by 1 to add in iem result

!  results stored in common/cobs_param/ icobs_st, nobs_st, obs_st(10,1) ?

      write(*,*) 'obs_st(1 in setobs_st =', obs_st(1,nobs_st)    ! for checking
      write(*,*) 'obs_st(2 in setobs_st =', obs_st(2,nobs_st)
      write(*,*) 'obs_st(3 in setobs_st =', obs_st(3,nobs_st)
      write(*,*) 'obs_st(4 in setobs_st =', obs_st(4,nobs_st)
      write(*,*) 'obs_st(5 in setobs_st =', obs_st(5,nobs_st)
      write(*,*) 'obs_st(6 in setobs_st =', obs_st(6,nobs_st)
      write(*,*) 'obs_st(7 in setobs_st =', obs_st(7,nobs_st)
      write(*,*) 'obs_st(8 in setobs_st =', obs_st(8,nobs_st)
      write(*,*) 'obs_st(9 in setobs_st =', obs_st(9,nobs_st)
