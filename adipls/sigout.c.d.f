...
!      integer :: ii
c
      common/rhsdat/ el,ell,alb,els,el1,sigcom,anres,perfac,data(8),  !sigcom? or sig
     *  aa(iaa,1)
!      real(dp) :: ell,alb,els,el1,sig,anres,perfac,data(8), aa(iaa,1)
!      integer :: el
c
!      common/rotdat/ em, irotsl, nsem, dems   ! added by me, uncomment if passing as subroutine arg
!      integer :: em, irotsl, nsem, dems
      integer, intent(in) :: iem, irotsl   ! comment this if passing with commons, not need if doing implicit dp
c
...
c
      write(*,*) 'Subroutine sigout called'

      write(*,*) 'Show me irotsl, iem, sig in sigout'
      write(*,*) irotsl, iem, sig
...
c
     ...
     *  iem, irotsl)  ! pass em, irots as arguments
c
...
c
c  adding em (azimuthal order) to csummm
c
      write(*,*) 'Show me irotsl and iem in sigout'
      write(*,*) irotsl, iem
c
      if(irotsl.eq.1) then
	write(*,*) 'Entering iem into cs(38) in sigout'
	csummm(38)=iem  ! cs(38) in real
      else
	csummm(38)=0.d0
      end if

      write(*,*) 'Show me cs(38) in sigout'  ! checking
      write(*,*) csummm(38)
c
...
c
      if (irotkr.gt.0) write(*,*) 'calling rotker'
...
!      call setobs_st(csummm,icsumm,iriche)
      ...
      write(*,*) 'Show me iem before calling setobs_st'
      write(*,*) iem

    ! adding iem to setobs_st arg just for checking, can be removed
      call setobs_st(csummm,icsumm,iriche,iem)
c
c  test for special output
!  calling adipls_support_procs in mesa for output !!!
!  results in terms of obs_st(*,nobs_st) returned from setobs_st call
!  moved to spcout_adi through commons /cobs_param/ & /csumma/
c
      write(*,*) 'Show me iem, ispcpr before calling spcout_adi'
      write(*,*) iem, ispcpr
c
